...
| Hosted workers | NodePool `workers`, `replicas: 2`, DHCP guest IPs from OPNsense on `172.16.3.0/24` |
...
The worker NodePool intentionally has no `spec.config` ConfigMap. Workers use DHCP from OPNsense on VLAN12, keep the generated KubeVirt/Machine names, and therefore do not need per-node hostname, static NetworkManager config, SSH key injection, CRI-O enablement, or manual providerID patching.
...
- hosted worker nodes from NodePool `workers` are `Ready` with DHCP addresses from `172.16.3.0/24`
- NodePool `workers` has `CURRENT NODES=2`
- Worker Machines have matching generated `NODENAME` and `PROVIDERID`
